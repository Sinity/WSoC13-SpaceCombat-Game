    ...
    sf::Uint8* CreateMask(const sf::Texture* tex, const sf::Image& img)
    {
        ...
        for(unsigned int y = 0; y < tex->getSize().y; y++)
            for(unsigned int x = 0; x < tex->getSize().x; x++)
                mask[y*tex->getSize().x + x] = img.getPixel(x, y).a;
...
bool PixelPerfectTest(const sf::Sprite& Object1, const sf::Vector2i _position, sf::Uint8 AlphaLimit)
{
    sf::Vector2f position = {(float)_position.x, (float)_position.y};
    if (Object1.getGlobalBounds().contains(position))
    {
        sf::IntRect O1SubRect = Object1.getTextureRect();
        sf::Uint8* mask1 = bitmasks.GetMask(Object1.getTexture());
        sf::Vector2f o1v = Object1.getInverseTransform().transformPoint(position.x, position.y);
